...
class Scaper:
    def __init__(
        ...
        if self.DCASE_format:
            self.fg_labels = {
                l: __DCASE_SOUND_EVENT_CLASSES__[l] for l in fg_label_list if l in __DCASE_SOUND_EVENT_CLASSES__
            }
        else:
    ...
    def get_path_to_room_ambient_noise(self):
        path_to_ambient_noise_files = os.path.join(
            ...
            '/nas/home/fferreri/SpatialScaper/datasets/rir_datasets', __PATH_TO_AMBIENT_NOISE_FILES__
        )
    ...
    def synthesize_events_and_labels(self, all_irs, all_ir_xyzs, out_audio):
        ...
        for ievent, event in enumerate(events):
            ...
            xS = event_scale * xS
            power_signal = np.mean(xS ** 2)  # Potenza del rumore
            print(power_signal)
    ...
    def calculate_snr(self, signal, noise):
        ...
        Args:
            signal (np.array): Segnale originale.
            noise (np.array): Rumore sovrapposto al segnale.

        Returns:
            float: SNR in decibel (dB)
        """
        power_signal = np.mean(signal ** 2)  # Potenza del segnale
        print(power_signal)
        power_noise = np.mean(noise ** 2)  # Potenza del rumore
        print(power_noise)

        if power_noise == 0:
            return np.inf  # Evita divisioni per zero
    ...
    def get_background_noise(self, out_audio):
        for ievent, event in enumerate(self.bg_events):
            ...
            print(f"mean background: {np.mean(np.abs(ambient))}")
            print(f"event.snr: {event.snr}, scale: {scale}")
            snr=self.calculate_snr(out_audio, scale * ambient)
            print(snr)
